"""Helpers for §7.2 Data & Environment page — kept separate so 02_data.py stays ≤150 LOC."""
...
def roll_trajectory(sdk: WorkoutSDK, days: int = 28) -> tuple[list[State], list[dict]]:
...
def handle_metrics(handle: LogbookHandle) -> dict[str, str]:
...
def load_section(state: GUIState, sdk: WorkoutSDK) -> LogbookHandle | None:
    ...
    with cols[0]:
        if st.button(
            ...
            states, infos = roll_trajectory(sdk, days=int(handle.n_days))
...
def heatmap_section(trajectory: list[State]) -> None:
...
def reward_section(state: GUIState, infos: list[dict]) -> None:
