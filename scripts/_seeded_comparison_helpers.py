...
def train_reinforce_seed(seed: int, n_episodes: int) -> np.ndarray:
...
def train_a2c_seed(seed: int, n_episodes: int) -> np.ndarray:
...
def run_sweep(n_seeds: int, n_episodes: int, base_seed: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    for i, seed in enumerate(seeds):
        ...
        r_stack[i] = train_reinforce_seed(seed, n_episodes)
        t_r = time.perf_counter() - t0
        t0 = time.perf_counter()
        a_stack[i] = train_a2c_seed(seed, n_episodes)
