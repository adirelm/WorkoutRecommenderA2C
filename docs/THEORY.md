...
where $f_\phi$ is an LSTM whose hidden state $h_t$ carries the relevant history. Once trained, $f_\phi$ is **frozen** and becomes the transition kernel that REINFORCE and A2C roll out against in Phases 4 and 5 — a learned simulator standing in for the unknown true MDP transition [2, 7].

**How this maps to `src/`.** `src/model/lstm_world.py::LSTMWorldModel` is $f_\phi$. Architecturally it concatenates the 12-dim state with an 8-dim learned action embedding (action embedding is preferred over one-hot per Ha & Schmidhuber 2018) into a 20-dim input, passes it through `nn.LSTM(hidden=64, num_layers=1)`, and projects the last hidden state through a `Linear(64 → 12)` head. `src/model/lstm_trainer.py::LSTMTrainer` supervises $f_\phi$ with MSE between predicted and observed $s_{t+1}$ on a sliding-window dataset built by `src/model/dataset.py::build_windows`. The trajectory is built by `src/model/program_trajectory.py::generate_program_trajectory`, which loads the chosen **real Kaggle program** (Optimized PHUL, §7.2.4) via `src/data/program_loader.py`, aggregates it to daily sessions, and rolls a deterministic trainee along that real schedule. `src/model/world_model_builder.py` fits + freezes $f_\phi$ and wraps it in `src/model/lstm_env_adapter.py::LSTMEnvAdapter`, which is injected as the `WorkoutEnv` transition provider so REINFORCE/A2C roll out against the frozen LSTM (not the analytic trainee).
...
**Honest limitation.** $f_\phi$ is trained on the *prescribed* content of the real Kaggle PHUL program (its day-level action sequence and Σ(sets·reps) volumes) — the physiological response (fatigue / soreness / readiness) is still supplied by the deterministic trainee rules, because the dataset carries no biological outcomes. So the LSTM learns the rhythm of a real program filtered through a simulated body, not measured trainee dynamics. Brief §7.6 Q4 makes this caveat explicit.
...
**How this maps to `src/`.** `src/services/base_trainer.py::BaseTrainer.rollout_episode` (driven by `REINFORCETrainer.train`) runs one episode against the LSTM-backed `WorkoutEnv` (built by `world_model_builder.build_lstm_env`), collects $(s_t, a_t, r_t)$ tuples, computes $G_t$ via `src/services/reinforce_helpers.py::compute_returns`, and applies the cross-entropy surrogate loss via `reinforce_loss`. The world model is frozen (`model.freeze()` → `requires_grad=False`) before being wrapped in `src/model/lstm_env_adapter.py::LSTMEnvAdapter` — the policy gradient never flows into the LSTM.
