...
| E4 | REINFORCE-vs-A2C head-to-head (DA4), both over the frozen LSTM env | `scripts/run_seeded_comparison.py` — 10 seeds × 50 episodes; per-seed last-5-episode tail; Welch's two-sample t-test | `results/figures/comparison.png` + `results/comparison_seeded.json` (10 seeds × 50 ep). Per-seed last-5-ep tail means: **REINFORCE = +5.303 ± σ=0.817**, **A2C = +5.083 ± σ=0.873**. Welch's t = **−0.551, p = 0.588** → **NOT significant at α = 0.05**; ±1σ bands overlap. | **NOT SIGNIFICANT.** Over the real-PHUL LSTM env the two algorithms are statistically indistinguishable at 10 seeds × 50 episodes (REINFORCE's mean is marginally higher; A2C's per-episode trace is slightly smoother). **Honest conclusion: no statistical claim of A2C > REINFORCE on this env.** |
...
- **Publication-grade seed/episode budget**: E4 uses 10 seeds × 50 episodes and computes Welch's t (p = 0.588, not significant). A publication-grade study would want 30+ seeds × 500 episodes.
...
- **Comparison against random/heuristic baseline**: §7.6 Q3 asks REINFORCE-vs-A2C; we have no random-policy baseline to anchor the absolute magnitude.
...
| **Setup** | `scripts/run_lambda_sensitivity.py` — λ_1 ∈ {0.5, 1.0, 1.5, 2.0, 3.0} × λ_2 ∈ {0.25, 0.5, 1.0, 1.5, 2.0}; per cell: train REINFORCE **over the frozen real-PHUL LSTM env** for 30 episodes × 3 seeds (42, 123, 2026), record the per-seed last-5-episode mean. **25 cells × 3 seeds = 75 runs, ~24 s wall-clock.** |
