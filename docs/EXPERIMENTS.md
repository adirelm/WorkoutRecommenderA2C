...
| E8 | Live-training per-episode chart-update callback slows training by >10% | REINFORCE, 50 episodes, seed=42, measured wall-clock with vs without the chart-update callback; report (t_with − t_without) / t_without as % overhead | Overhead measured below the 10% threshold (see results/figures/live_training_overhead.png); callback cost dominated by Streamlit rerun, not policy step | REJECTED (>10% claim) — overhead is non-trivial but under threshold; callback acceptable for live UX |
